...
2. **ETL stage:** replicate the exact job locally with the hotglue CLI (`hotglue etl setup-local-run <job>` then `hotglue etl local-run`) — see the `local-job-debugging` skill (`.claude/skills/local-job-debugging/`) in this repo.
